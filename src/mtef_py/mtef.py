    ...
    def makeLatex(self, ast):
        ...
        if ast.tag == RecordType.ROOT:
            ...
            if sChar:
                ...
                # 尝试另一种后缀（带/mathmode ↔ 不带），兼容不同 typeface 编码
                altKey = 'char/0x%s%s' % (hexCode, '' if hexExtend else '/mathmode')
                if altKey != hexKey:
                    sChar = Chars.get(altKey)
                    if sChar:
                        char = sChar
            if not sChar:
                ...
                if sChar:
                    ...
                    # 完全未知的符号：输出 Unicode 码点提示，不裸输出原字符
                    char = '{\\text{[U+%s]}}' % hexCode
            ...
            if tmpl.selector == SelectorType.tmANGLE:
                ...
                # 检测是否为方程组模式：左花括号 + 空右括号 → 使用 \begin{cases}
                is_equation_system = (
                    leftSlot.strip() == '{' or leftSlot.strip() == '\\{'
                ) and rightSlot.strip() == '.'

                if is_equation_system:
                    # 尝试展平：如果 main 是 PILE 或 包含 MATRIX，直接取子行构建 cases
                    # 避免 \begin{cases}\begin{gathered/array}...\end{gathered/array}\end{cases} 冗余嵌套
                    cases_lines = None

                    if mainAST is not None and mainAST.tag == RecordType.PILE:
                        # PILE 直接取子行
                        cases_lines = []
                        for child in mainAST.children:
                            line_latex, _ = self.makeLatex(child)
                            cases_lines.append(line_latex)

                    elif mainAST is not None:
                        # 尝试递归查找 MATRIX（常见模式：LINE → MATRIX）
                        def _find_matrix(ast):
                            if ast is None:
                                return None
                            if ast.tag == RecordType.MATRIX:
                                return ast
                            for child in (ast.children or []):
                                result = _find_matrix(child)
                                if result is not None:
                                    return result
                            return None

                        matrix_ast = _find_matrix(mainAST)
                        if matrix_ast is not None:
                            cases_lines = []
                            for i, child in enumerate(matrix_ast.children):
                                if i == 0:
                                    continue  # 跳过 nil LINE
                                line_latex, _ = self.makeLatex(child)
                                cases_lines.append(line_latex)

                    if cases_lines is not None and len(cases_lines) > 1:
                        buf += '\\begin{cases} '
                        for li, line in enumerate(cases_lines):
                            if li > 0:
                                buf += ' \\\\ '
                            buf += line
                        buf += ' \\end{cases}'
                    else:
                        # 单行或无法展平的情况，直接用 mainSlot
                        buf += '\\begin{cases} %s \\end{cases}' % mainSlot
                else:
                    # 组装公式（保留原有逻辑处理其他花括号场景）
            ...
            pile = ast.value
            # 根据 PILE 的 halign 属性选择合适的 LaTeX 对齐环境
            # halign: 0=left, 1=center, 2=right
            halign_map = {0: 'l', 1: 'c', 2: 'r'}
            halign_char = halign_map.get(pile.halign, 'c')
            ...
            # 多行 PILE：包裹在 aligned 环境中，支持多列对齐点 &
            # 对齐方式通过 PILE 的 ruler/tabStop 定义，无 tab 则每行整体对齐
            env = 'aligned'
            if halign_char == 'c':
                env = 'gathered'
            buf += ' \\begin{%s} ' % env
            ...
            matrix = ast.value
            matrixCol = int(matrix.cols)
            matrixRow = int(matrix.rows)

            # 根据 h_just 推导列对齐
            # h_just: 0=left, 1=center, 2=right
            h_map = {0: 'l', 1: 'c', 2: 'r'}
            col_align = h_map.get(matrix.h_just, 'c')
            col_spec = '{' + col_align * matrixCol + '}'

            idx = 0
            total_items = len(ast.children)
            for _ast in ast.children:
                _latex, _ = self.makeLatex(_ast)

                if idx == 0:
                    # 第一个子节点是 nil LINE（MTEF 格式占位），跳过内容
                    # 只输出 array 开始标记
                    buf += " \\begin{array}%s " % col_spec
                    idx += 1
                    continue

                buf += _latex

                # 判断是否需要行分隔符：不是最后一项，且在当前行末尾
                is_last = (idx == total_items - 1)
                if not is_last and idx % matrixCol == 0:
                    buf += " \\\\ "
                elif not is_last:
                    buf += " & "
                idx += 1
